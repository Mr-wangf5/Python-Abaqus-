...
# 知识点3： 箍筋集合（元组）
# 知识点4： 切分实体简写（遍历列表）
# 知识点5： 关键词（main-solve, main-secondary）
...
# 混凝土粱建模
Partb = m.ConstrainedSketch(name='ConBeam-sketch', sheetSize=3000.0)
...
a = m.rootAssembly
# 布置混凝土粱
Part01 = a.Instance(name='Part-beam', part=Partbeam, dependent=ON)
# 布置箍筋
Buzhigujin = []
for i in range(0, Ns):
    p = a.Instance(name='gujin-00{}'.format(i), part=Stirr, dependent=ON)
    Buzhigujin.append(p)

for i, item in enumerate(Buzhigujin):
...
##知识点5：关键词（main-secondary；main-secondary）
a = m.rootAssembly
region1 = a.surfaces['Surf-BC-1-C']
region2 = a.surfaces['Surf-BC-1-G']
m.Tie(name='Constraint-1', main=region1, secondary=region2, positionToleranceMethod=COMPUTED, adjust=ON, tieRotations=ON, thickness=ON)
region1 = a.surfaces['Surf-BC-2-C']
region2 = a.surfaces['Surf-BC-2-G']
m.Tie(name='Constraint-2', main=region1, secondary=region2, positionToleranceMethod=COMPUTED, adjust=ON, tieRotations=ON, thickness=ON)
region1 = a.surfaces['Surf-BC-3-C']
region2 = a.surfaces['Surf-BC-3-G']
m.Tie(name='Constraint-3', main=region1, secondary=region2, positionToleranceMethod=COMPUTED, adjust=ON, tieRotations=ON, thickness=ON)
region1 = a.surfaces['Surf-BC-4-C']
region2 = a.surfaces['Surf-BC-4-G']
m.Tie(name='Constraint-4', main=region1, secondary=region2, positionToleranceMethod=COMPUTED, adjust=ON, tieRotations=ON, thickness=ON)
##########################################################################################################################################
# 内置
a = m.rootAssembly
